...
RoboNix 检索增强 Toolkit 为现有具身模型提供一个独立部署的**经验记忆与复用
Skill**。系统将历史机器人执行数据转化为可检索记忆，根据当前场景和任务返回相关
动作轨迹，使原有策略无需被替换即可复用历史执行经验。

检索轨迹是候选动作，而不是无条件下发的机器人指令。候选可以进入校验与执行链路，
也可以在不可信时被拒绝并回退到原始策略。当前版本支持单视角检索和融合第三视角、
腕部视角的双视角 Mix 流程，并提供构库、服务、维护与评测工具。
...
- **2026-07-19**：🆕 将工具包重构为系统级经验记忆与复用 Skill，补充能力效果、
  模型支持和中英文文档。
...
本工具包以独立 Skill Toolkit（技能工具包）的形式交付，作为经验记忆服务和可复用自定义服务接入 RoboNix。Scene 提供当前观测与任务上下文，Atlas 发现能力提供方，Nexus 传输多模态引用，Pilot 使用返回的候选轨迹，而不把数据库逻辑写入 RoboNix 核心。
