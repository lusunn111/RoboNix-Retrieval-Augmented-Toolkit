#!/usr/bin/env python3
"""
Build incremental Qdrant backups by augmenting base collections with BridgeV2 episodes.

This script will:
1) Restore Qdrant from backup_base
2) Add N episodes per existing collection from BridgeV2 TFRecords
3) Backup to backup_base+N (without touching the latest symlink by default)
4) Repeat for N in [10, 20, 40, 80] by incremental additions

Usage:
  python build_bridge_increments.py \
...
def _pick_first_key(feature_keys: Iterable[str], preferred: List[str]) -> Optional[str]:
    for key in preferred:
        if key in feature_keys:
            return key
    for key in sorted(feature_keys):
        if "steps/observation/image" in key:
            return key
    return None


def parse_bridge_episode(raw_bytes: bytes) -> Tuple[List[bytes], np.ndarray, str]:
    """Parse a BridgeV2 episode from TFRecord Example."""
    example = tf.train.Example()
    example.ParseFromString(raw_bytes)
    features = example.features.feature

    image_key = _pick_first_key(
        features.keys(),
        [
    ...
    if not image_key:
        raise ValueError("No image key found in episode")

    image_bytes_list = list(features[image_key].bytes_list.value)
    ...
    return image_bytes_list, actions, instruction


def generate_embedding(image: Image.Image, instruction: str, url: str) -> Optional[List[float]]:
    try:
        buf = BytesIO()
...
def upsert_episode(
    ...
    image_bytes_list, actions, instruction = parse_bridge_episode(raw_bytes)
    num_steps = len(image_bytes_list)
    if num_steps == 0:
        return 0
    ...
    if actions.shape[0] != num_steps:
        if actions.shape[0] == 1:
            ...
            image_bytes_list = image_bytes_list[:min_steps]
    ...
    for step_idx in range(num_steps):
        try:
            image = Image.open(BytesIO(image_bytes_list[step_idx])).convert("RGB")
        except Exception:
            continue

        embedding = generate_embedding(image, instruction, embedding_url)
        if embedding is None:
            continue
        if len(embedding) != openvla_dim:
            logging.warning(
                f"Embedding dim mismatch: got {len(embedding)}, expected {openvla_dim}"
            )
...
def _filter_libero_collections(collection_names: List[str]) -> List[str]:
    prefixes = [
        "libero_goal_task_",
...
def backup_collections(
    ...
    backup_subdir = backup_root / f"backup_{note}"
    if backup_subdir.exists():
        shutil.rmtree(backup_subdir)
    backup_subdir.mkdir(parents=True, exist_ok=True)

    collections = client.get_collections().collections
    collection_names = _filter_libero_collections([c.name for c in collections])
...
def get_collection_names(client: QdrantClient) -> List[str]:
    collections = client.get_collections().collections
    names = _filter_libero_collections([c.name for c in collections])
    names.sort()
    return names
...
def build_increments(config: BuildConfig, targets: List[int]) -> None:
    ...
    logging.info(f"Restoring base from {base_backup}")
    ...
    for target in targets:
        ...
        logging.info(f"Adding {delta} episodes per collection (target {target})")
        ...
        with tqdm(total=total_needed, desc=f"Insert base+{target}", unit="episode") as pbar:
        ...
        logging.info(f"Backing up to {backup_note}")
        with tqdm(total=len(collection_names), desc=f"Backup {backup_note}", unit="collection") as pbar:
            backup_subdir = backup_root / f"diff_backup_{backup_note}"
            if backup_subdir.exists():
                shutil.rmtree(backup_subdir)
            backup_subdir.mkdir(parents=True, exist_ok=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build incremental Qdrant backups with BridgeV2 episodes"
    )
    parser.add_argument(
        "--bridge-dir",
        type=str,
        default=os.getenv("BRIDGE_DATASET_ROOT", str(Path(__file__).resolve().parents[2] / "data" / "bridgev2" / "1.0.0")),
        help="BridgeV2 dataset directory",
    )
    parser.add_argument(
        "--backup-root",
        type=str,
        default=os.getenv("QDRANT_BACKUP_ROOT", str(Path(__file__).resolve().parents[2] / "data" / "qdrant_backups")),
    ...
    parser.add_argument(
        "--update-latest",
        action="store_true",
        help="Update latest symlink to the last backup",
    )
...
def main() -> int:
    ...
    config = BuildConfig(
        ...
        embedding_url=rt_config.server.embedding_url,
        openvla_dim=rt_config.retrieval.openvla_dim,
    ...
    logging.info("Starting incremental build")
