...
class ProcessingConfig:
    ...
    batch_size: int = config.dataset.batch_insert_size
    max_episodes: int = config.dataset.max_episodes
    min_episode_length: int = config.dataset.min_episode_length
    
    # Embedding dimensions
    openvla_image_dim: int = config.retrieval.openvla_dim
    openvla_text_dim: int = config.retrieval.openvla_dim  # Assuming same dim for now
    clip_image_dim: int = config.retrieval.clip_dim
    clip_text_dim: int = config.retrieval.clip_dim
...
class DatasetProcessor:
    ...
    def _extract_action(self, action_data) -> np.ndarray:
        ...
        """
        # Handle direct 7-dim action (Numpy or Tensor)
        if isinstance(action_data, (np.ndarray, tf.Tensor)):
            if hasattr(action_data, 'shape') and action_data.shape[-1] == 7:
    ...
    def _get_dataset_path(self, dataset_name: str) -> str:
        ...
        # Check for local LIBERO datasets first
        libero_base_path = Path("/path/to/rtcache/libero/datasets--openvla--modified_libero_rlds/snapshots/6ce6aaaaabdbe590b1eef5cd29c0d33f14a08551")
        local_path = libero_base_path / dataset_name / "1.0.0"
        
        if local_path.exists():
            self.logger.info(f"Found local dataset at: {local_path}")
    ...
    def _get_observation_keys(self, builder, is_rlds: bool) -> Dict:
        ...
        if is_rlds:
            ...
            step_keys = list(builder.info.features['steps'].keys())
        else:
        ...
        # If not in observation, check steps (common in some RLDS datasets)
        if not text_key and is_rlds:
            text_key = next((k for k in possible_text_keys if k in step_keys), None)
    ...
    def _extract_episode_text(self, first_step: Dict, obs_keys: Dict) -> Optional[str]:
        ...
        text_key = obs_keys['text']
        if not text_key:
            return None
            
        text_data = None
        if 'observation' in first_step and text_key in first_step['observation']:
            text_data = first_step['observation'][text_key]
        elif text_key in first_step:
            text_data = first_step[text_key]
        ...
        if isinstance(text_data, bytes):
            ...
            return text_data
        elif hasattr(text_data, 'numpy'):
            return text_data.numpy().decode('utf-8')
