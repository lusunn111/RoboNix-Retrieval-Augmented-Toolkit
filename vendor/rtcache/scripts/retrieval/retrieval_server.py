...
# Monkeypatch QdrantClient.search for compatibility with newer qdrant-client versions
# llama-index-vector-stores-qdrant 0.1.1 expects .search(), but qdrant-client 1.16.2 removed it.
from qdrant_client import QdrantClient
if not hasattr(QdrantClient, 'search'):
    print("[WARNING] Monkeypatching QdrantClient.search to use query_points...")
    def search_shim(self, collection_name, query_vector, **kwargs):
        # Map query_vector to query
        if 'query' not in kwargs:
            kwargs['query'] = query_vector
        
        # Call query_points
        response = self.query_points(collection_name=collection_name, **kwargs)
        # query_points returns QueryResponse, search expects List[ScoredPoint]
        return response.points
        
    QdrantClient.search = search_shim
...
if EMBEDDING_TYPE == "clip":
    QDRANT_COLLECTION = "clip_image_collection"
    CENTROID_COLL_NAME = "clip_image_collection"
else:
    QDRANT_COLLECTION = "image_collection"
    CENTROID_COLL_NAME = "image_collection"
    ...
    def parse_to_query_result(self, search_result):
        ...
        for point in search_result:
            ...
            if text_content is None:
                text_content = " "  # Use a space or placeholder if text is missing
...
doc_cache = {}
# Fix projection: Always include logical_id as it is required for retrieval lookups
projection = {ID_FIELD: 1, "raw_action": 1, "logical_id": 1, "id": 1}
if ID_FIELD != "_id":
    projection["_id"] = 0

cursor = mongo_coll.find({}, projection)
count = 0
for doc in cursor:
    # Prefer logical_id as key because gather_consecutive_actions uses it
    key = doc.get("logical_id")
    if not key:
        key = doc.get("id")
    if not key:
    ...
    if key:
        doc_cache[str(key)] = doc
        count += 1
print(f"### Loaded {count} documents into doc_cache.\n")
if count > 0:
    print(f"### Sample doc_cache keys: {list(doc_cache.keys())[:5]}")
else:
    print("### WARNING: doc_cache is empty!")
...
def gather_consecutive_actions(sample_id: str, n_steps: int):
    ...
    if len(parts) != 2:
        print(f"  [DEBUG] gather_consecutive_actions: Invalid sample_id format: {sample_id}")
        return []
    prefix, step_str = parts
    try:
        base_step = int(step_str)
    except ValueError:
        print(f"  [DEBUG] gather_consecutive_actions: Invalid step number in: {sample_id}")
        return []
    seq = []
    for offset in range(n_steps + 1):
        ...
        if not doc:
            print(f"  [DEBUG] gather_consecutive_actions: Missing doc for {doc_id} (offset {offset})")
            # Try to print keys that are close
            # close_keys = [k for k in doc_cache.keys() if k.startswith(prefix)]
            # print(f"  [DEBUG] Available keys with prefix {prefix}: {close_keys[:5]}...")
            return []
        raw_act = doc.get("raw_action", ZERO_ACTION)
        if raw_act == ZERO_ACTION:
            print(f"  [DEBUG] gather_consecutive_actions: ZERO_ACTION for {doc_id}")
            return []
        # Debug logs
...
def qdrant_search_image(pil_image: Image.Image, topk=10):
    ...
    print(f"#### DONE Vector SEARCH. Found {len(results.nodes) if results.nodes else 0} nodes.")
    if results.nodes:
        for i, node in enumerate(results.nodes):
            print(f"  Result {i}: ID={node.node_id}, Score={results.similarities[i] if results.similarities else 'N/A'}")
...
def predict_closest_dataset_with_centroids(query_tensor):
    ...
    # qdrant_client.search is deprecated in v1.10.0+, use qdrant_client.query_points or qdrant_client.search is actually available but maybe the client version is different?
    # Actually, qdrant_client.search IS the correct method for QdrantClient.
    # However, if qdrant_client is an instance of QdrantClient from qdrant_client package, it should have search.
    # Let's check if qdrant_client is initialized correctly.
    # It is initialized as: qdrant_client = QdrantClient(host=QDRANT_HOST, port=QDRANT_PORT, timeout=300)
    
    # If the error says 'QdrantClient' object has no attribute 'search', it might be an older version or a very new version with breaking changes?
    # Or maybe it's a sync/async client issue.
    
    # Let's try to use the search method if it exists, otherwise try query_points (new API) or just skip if not critical.
    # But wait, 'search' is the standard method.
    
    # Let's check if we can use the LlamaIndex store instead, or just fix the call.
    # The error is very specific: AttributeError: 'QdrantClient' object has no attribute 'search'
    
    # Wait, maybe qdrant_client is NOT what we think it is?
    # In line 238: qdrant_client = QdrantClient(host=QDRANT_HOST, port=QDRANT_PORT, timeout=300)
    
    # Let's try to use `search` from `qdrant_client` directly.
    # If the user has `qdrant-client` < 1.0, the method might be different?
    # Or if they have a very new version.
    
    # Actually, let's look at the imports.
    # from qdrant_client import QdrantClient
    
    # If the error persists, maybe we can use `image_store.client`?
    # image_store.client IS qdrant_client.
    
    # Let's try to use a safe approach.
    try:
        if hasattr(qdrant_client, 'search'):
            ...
            # For now, let's just return "unknown" and log a warning if we can't search.
            # But wait, we need this to work.
            
            # Maybe the collection doesn't exist? No, that would be a different error.
            # The error is on the OBJECT attribute.
            
            # Let's assume it might be `query_points` in newer versions?
            # Or maybe `search_points`?
            
            # Let's just return "unknown" for now to unblock the user, as this seems to be an optional feature (finding closest dataset).
            print("WARNING: qdrant_client.search not found. Skipping centroid search.")
            return "unknown"
        ...
        print(f"WARNING: Failed to search centroids: {e}")
        return "unknown"
...
def parse_episode_and_step(sample_id):
    ...
    'libero_goal_no_noops_308_36' => ep='308', st='36'
    """
...
def pipeline():
    try:
        ...
        final_actions = []
        if LOAD_RTCACHE:
            print(f"##### Processing {len(image_hits)} image hits for consecutive actions...")
            for hit in image_hits:
                sample_id = hit["payload"].get("logical_id", None)
                if not sample_id:
                    print("  Skipping hit with no logical_id")
                    continue

                # gather consecutive
                print(f"  Checking consecutive actions for {sample_id}...")
                ...
                if seq:
                    ...
                    print(f"  Sequence not found or incomplete for {sample_id}")
        ...
        # Add specific trajectories if available
        if LOAD_RTCACHE:
            response["rtcache_trajectory"] = rtcache_trajectory
        if LOAD_VINN:
