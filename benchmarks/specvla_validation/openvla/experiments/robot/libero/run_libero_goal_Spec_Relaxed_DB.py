"""
run_libero_goal_Spec_Relaxed_DB.py

Runs a model in a LIBERO simulation environment with alternating DB retrieval and Speculative Decoding generation.

Usage:
    python experiments/robot/libero/run_libero_goal_Spec_Relaxed_DB.py \
...
class GenerateConfig:
    ...
    #################################################################################################################
    db_steps: int = 2                                # Number of consecutive DB retrieval actions
    model_steps: int = 1                              # Number of consecutive model generation actions

    #################################################################################################################
    # Online DB insertion (successful rollouts)
    #################################################################################################################
    enable_online_db_insert: bool = False
    ...
    online_db_insert_max_steps: int = -1  # -1 means no limit
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    rollout_inserter = None
    if cfg.enable_online_db_insert:
    ...
    target_dir = os.path.join(base_dir, f"libero_goal_Spec_DB_N{cfg.db_steps}_M{cfg.model_steps}")
    ...
    local_log_timefilepath = os.path.join(target_dir, run_id + "libero_goal_Spec_DB.json")
    ...
    if cfg.enable_online_db_insert:
        msg = (
            f"Online DB insert enabled: qdrant={cfg.online_db_qdrant_url} "
            f"embedding={cfg.online_db_embedding_server_url} "
            f"dataset_name={cfg.online_db_dataset_name} stride={cfg.online_db_insert_stride} "
            f"max_steps={cfg.online_db_insert_max_steps}"
        )
        print(msg)
        log_file.write(msg + "\n")
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
            ...
            while t < max_steps + cfg.num_steps_wait:
                try:
                    ...
                    if use_db_mode:
                        # ========================================
                        # DB Retrieval Mode - ONLY DB, NO SD
                        # ========================================
                        if len(action_queue) > 0:
                            # Use cached action from previous DB call, no timing needed
                            action = action_queue.pop(0)
                            # Don't record time for cached actions
                        else:
                        ...
                        # ASSERT: In DB mode, we should NEVER call get_action (SD)
                        # DB mode is complete, proceed to execute action
                        
                    else:
                        # ========================================
                        # Model Generation Mode - ONLY SD, NO DB
                        # ========================================
                        if cfg.model_steps == 0:
                        ...
                        # ONLY record time for get_action (SD inference)
            ...
            if done and cfg.enable_online_db_insert and rollout_inserter is not None:
                try:
                    ...
                    print(f"[OnlineInsert] Failed: {e}")
                    log_file.write(f"[OnlineInsert] Failed: {e}\n")
                    log_file.flush()
        ...
        timing_msg = f"[Timing] task {task_id}: DB retrieval: {num_db_calls} calls, avg {avg_db_time:.6f}s | Model (Spec): {num_model_calls} calls, avg {avg_model_time:.6f}s | Total steps: {task_total_steps} ({task_episodes} episodes)"
        print(timing_msg)
    ...
    for stat in task_timing_stats:
        msg = f"[Timing] task {stat['task_id']}: DB retrieval: {stat['num_db_calls']} calls, avg {stat['avg_db_time']:.6f}s | Model (Spec): {stat['num_model_calls']} calls, avg {stat['avg_model_time']:.6f}s | Total steps: {stat['total_steps']} ({stat['num_episodes']} episodes)"
        print(msg)
        log_file.write(msg + "\n")
    ...
    print(f"  Model (Spec): {total_model_calls} calls, average time: {overall_avg_model_time:.6f}s")
    ...
    log_file.write(f"  Model (Spec): {total_model_calls} calls, average time: {overall_avg_model_time:.6f}s\n")
