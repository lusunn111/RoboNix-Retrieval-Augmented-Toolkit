#!/bin/bash
# 运行基于综合指标的SD + Retrieval实验（Mix视角）
# 
# 决策逻辑：
# - 综合指标 > 0.143210：使用Retrieval策略
#   - Retrieval策略：2次verify(DB) + 1次noverify(AR)
#   - verify(DB)：直接使用检索的action
#   - noverify(AR)：使用AR生成（不验证）
# - 综合指标 <= 0.143210：使用SD (Speculative Decoding)
#
...
# - alpha = 0.5（1:1构造综合指标）
#
# 使用方法: bash run_libero_retrieval_verify_sd_retrieval_mix.sh
#
# 前置条件：
...
# 综合指标参数（新的归一化范围）
WINDOW_SIZE=5
COMPOSITE_THRESHOLD=0.143210  # 阈值
ALPHA=0.5  # 1:1权重
...
NUM_TRIALS=10
RUN_ID_NOTE="SD_Retrieval_Mix"

# 日志目录
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_DIR="./experiments/logs/sd_retrieval_mix_${TIMESTAMP}"
mkdir -p "$LOG_DIR"
