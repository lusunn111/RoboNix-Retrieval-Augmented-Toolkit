#!/bin/bash
# 运行脚本：使用DB检索和AR交替执行机制
# 使用方法: bash run_libero_goal_AR_DB.sh [--db_steps N] [--model_steps M]
#
# 前置条件：
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            echo "未知参数: $1"
            echo "用法: bash run_libero_spatial_AR_DB.sh [--db_steps N] [--model_steps M]"
            exit 1
            ;;
    esac
done
...
python openvla/experiments/robot/libero/run_libero_goal_AR_DB.py \
    ...
    --run_id_note "AR_DB_N${DB_STEPS}_M${MODEL_STEPS}" \
    --use_wandb False
...
if [ $? -eq 0 ]; then
    ...
    echo "✓ 运行完成！"
    echo "=========================================="
    echo "结果文件保存在: $SPECVLA_ROOT/openvla/specdecoding/test-speed/libero_spatial_AR_DB_N${DB_STEPS}_M${MODEL_STEPS}"
else
    echo ""
    echo "=========================================="
    echo "✗ 运行失败！"
