"""
run_libero_naive_DB.py

Runs pure DB retrieval (no model) in LIBERO simulation environment.

Usage:
    python experiments/robot/libero/run_libero_naive_DB.py \
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    target_dir = f"/path/to/SpecVLA/openvla/specdecoding/test-speed/{cfg.task_suite_name}_naive_DB"
    os.makedirs(target_dir, exist_ok=True)
    run_id = f"EVAL-{cfg.task_suite_name}-NaiveDB-{DATE_TIME}"
    if cfg.run_id_note is not None:
        run_id += f"--{cfg.run_id_note}"
    os.makedirs(cfg.local_log_dir, exist_ok=True)
    local_log_filepath = os.path.join(target_dir, run_id + ".txt")
    log_file = open(local_log_filepath, "w")
    local_log_timefilepath = os.path.join(target_dir, run_id + "_naive_DB.json")
    ...
    print(f"Mode: Pure DB Retrieval (No Model)")
    log_file.write(f"Mode: Pure DB Retrieval (No Model)\n")
