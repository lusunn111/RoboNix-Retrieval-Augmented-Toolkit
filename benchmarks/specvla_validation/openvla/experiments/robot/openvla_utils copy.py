...
# Global debug log file (set by run_libero_goal_Spec_Relaxed.py)
DEBUG_LOG_FILE = None
...
def get_vla_action(vla, processor, base_vla_name, obs, task_label, unnorm_key, return_hidden_states=False,return_time=False,center_crop=False,generate_mode=None,accept_threshold=None,return_topk_index=False,token=None,track_accept_length=False,db_action_slice=None,use_db_action_slice=False):
    ...
    if return_hidden_states:
        ...
        if use_db_action_slice:
            predict_kwargs['db_action_slice'] = db_action_slice
            predict_kwargs['use_db_action_slice'] = use_db_action_slice
        action,token,hidden = vla.predict_action(**inputs, **predict_kwargs)
    ...
    # Pass DB action slice parameters to predict_action
    predict_kwargs = {
        'unnorm_key': unnorm_key,
        'return_hidden_states': return_hidden_states,
        'do_sample': False,
    ...
    accept_lengths = None
    draft_tokens = None
    db_accept_lengths = None
    db_action_slices = None
    
    if track_accept_length:
        if isinstance(result, tuple):
            # Debug: log what we got from predict_action
            try:
                from experiments.robot import openvla_utils as openvla_utils_module
                if openvla_utils_module.DEBUG_LOG_FILE is not None:
                    openvla_utils_module.DEBUG_LOG_FILE.write(f"  [get_vla_action] predict_action returned {len(result)} values, use_db_action_slice={use_db_action_slice}\n")
                    openvla_utils_module.DEBUG_LOG_FILE.flush()
            except:
                pass
            
            if len(result) == 5 and use_db_action_slice:
                # DB action slice tracking enabled (no return_hidden_states)
                action, accept_lengths, draft_tokens, db_accept_lengths, db_action_slices = result
                # Debug log
                try:
                    from experiments.robot import openvla_utils as openvla_utils_module
                    if openvla_utils_module.DEBUG_LOG_FILE is not None:
                        openvla_utils_module.DEBUG_LOG_FILE.write(f"  [get_vla_action] Extracted db_accept_lengths={db_accept_lengths}, type={type(db_accept_lengths)}\n")
                        openvla_utils_module.DEBUG_LOG_FILE.flush()
                except:
                    pass
            elif len(result) == 7 and use_db_action_slice:
                # DB action slice tracking + return_hidden_states
                action, token_ids, hidden_states, accept_lengths, draft_tokens, db_accept_lengths, db_action_slices = result
            elif len(result) == 3:
                action, accept_lengths, draft_tokens = result
            elif len(result) == 5:
                # return_hidden_states is also True (no DB tracking)
                action, token_ids, hidden_states, accept_lengths, draft_tokens = result
                # Debug: this shouldn't happen if use_db_action_slice is True
                if use_db_action_slice:
                    try:
                        from experiments.robot import openvla_utils as openvla_utils_module
                        if openvla_utils_module.DEBUG_LOG_FILE is not None:
                            openvla_utils_module.DEBUG_LOG_FILE.write(f"  [get_vla_action] Warning: got 5 values but use_db_action_slice=True, DB data might be missing\n")
                            openvla_utils_module.DEBUG_LOG_FILE.flush()
                    except:
                        pass
            else:
                action = result[0] if len(result) > 0 else result
                accept_lengths = result[1] if len(result) > 1 else None
                draft_tokens = result[2] if len(result) > 2 else None
                # Debug: this shouldn't happen if use_db_action_slice is True
                if use_db_action_slice:
                    try:
                        from experiments.robot import openvla_utils as openvla_utils_module
                        if openvla_utils_module.DEBUG_LOG_FILE is not None:
                            openvla_utils_module.DEBUG_LOG_FILE.write(f"  [get_vla_action] Warning: got {len(result)} values but use_db_action_slice=True, DB data might be missing\n")
                            openvla_utils_module.DEBUG_LOG_FILE.flush()
                    except:
                        pass
        else:
    ...
    if return_time:
        if track_accept_length:
            # Check if db_accept_lengths was extracted (could be list or None)
            if use_db_action_slice and db_accept_lengths is not None:
                # DB action slice tracking enabled - use already extracted variables
                # Debug log
                try:
                    from experiments.robot import openvla_utils as openvla_utils_module
                    if openvla_utils_module.DEBUG_LOG_FILE is not None:
                        openvla_utils_module.DEBUG_LOG_FILE.write(f"  [get_vla_action] Returning 6 values: db_accept_lengths={db_accept_lengths}\n")
                        openvla_utils_module.DEBUG_LOG_FILE.flush()
                except:
                    pass
                return action, (end_time, start_time), accept_lengths, draft_tokens, db_accept_lengths, db_action_slices
            else:
                # Debug log
                try:
                    from experiments.robot import openvla_utils as openvla_utils_module
                    if openvla_utils_module.DEBUG_LOG_FILE is not None:
                        openvla_utils_module.DEBUG_LOG_FILE.write(f"  [get_vla_action] Returning 4 values: use_db_action_slice={use_db_action_slice}, db_accept_lengths is None={db_accept_lengths is None}\n")
                        openvla_utils_module.DEBUG_LOG_FILE.flush()
                except:
                    pass
            return action, (end_time, start_time), accept_lengths, draft_tokens
