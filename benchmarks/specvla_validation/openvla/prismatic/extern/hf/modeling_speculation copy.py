...
def _debug_log(msg):
    ...
    try:
        ...
        if openvla_utils.DEBUG_LOG_FILE is not None:
            openvla_utils.DEBUG_LOG_FILE.write(msg + "\n")
            openvla_utils.DEBUG_LOG_FILE.flush()
    except:
        pass  # Silently fail if log file is not available
...
class SpecVLAforActionPrediction(nn.Module):
    ...
    def __init__(
        ...
        # Load config from local path - handle both local files and HF Hub
        # Check if it's a local directory with config.json
        config_path = os.path.join(ea_model_path, "config.json") if os.path.isdir(ea_model_path) else ea_model_path
        if os.path.exists(config_path) or (os.path.isdir(ea_model_path) and os.path.exists(os.path.join(ea_model_path, "config.json"))):
            # Local path - use local_files_only to avoid HF Hub lookup
            try:
                config = EConfig.from_pretrained(ea_model_path, local_files_only=True)
            except Exception:
                # Fallback: load config.json directly
                config_json_path = os.path.join(ea_model_path, "config.json")
                with open(config_json_path, 'r') as f:
                    config_dict = json.load(f)
                config = EConfig(**config_dict)
        else:
            # Try as HF Hub repo
    ...
    def action_to_token_ids(self, action: np.ndarray) -> np.ndarray:
        """
        Convert action values in [-1, 1] range to token IDs.
        
        Args:
            action: Action array of shape (action_dim,) with values in [-1, 1]
            
        Returns:
            Token IDs array of shape (action_dim,)
        """
        # Clip action to valid range
        action = np.clip(action, a_min=-1.0, a_max=1.0)
        
        # Discretize using bins (same logic as action_tokenizer)
        # np.digitize returns indices in [1, n_bins], but we need [0, n_bins-1]
        discretized_action = np.digitize(action, self.bins)
        # Adjust: digitize returns [1, n_bins], we need [0, n_bins-1]
        discretized_action = np.clip(discretized_action - 1, a_min=0, a_max=len(self.bin_centers) - 1)
        
        # Convert to token IDs: token_id = vocab_size - discretized_action
        token_ids = self.vocab_size - discretized_action
    ...
    def predict_action(
        ...
        track_accept_length: bool = False,
        #accept_threshold=None,
        **kwargs: str
        ...
        # Extract DB action slice related parameters BEFORE the if statement
        # So they're available in the entire function scope
        db_action_slice = kwargs.pop('db_action_slice', None)
        use_db_action_slice = kwargs.pop('use_db_action_slice', False)
        ...
        if generate_mode == 'speculative':
            outputs = self.eagenerate(
                ...
                track_accept_length=track_accept_length,
       ...
       # exit()
        ...
        #print(outputs)
        # 获取生成的token IDs
        # Handle tracking data if track_accept_length is enabled
        accept_lengths_data = None
        draft_tokens_data = None
        db_accept_lengths_data = None
        db_action_slices_data = None
        # IMPORTANT: use_db_action_slice was already extracted above (before the if statement)
        # So it's available in the entire function scope - no need to get it from kwargs again
        
        if track_accept_length and isinstance(outputs, tuple):
            # Debug: log what we got from eagenerate
            _debug_log(f"  [predict_action] eagenerate returned {len(outputs)} values, use_db_action_slice={use_db_action_slice}")
            if len(outputs) == 5 and use_db_action_slice:
                # DB action slice tracking enabled
                # eagenerate returns: (input_ids, accept_lengths, draft_tokens_list, db_accept_lengths, db_action_slices)
                generated_ids, accept_lengths_data, draft_tokens_data, db_accept_lengths_data, db_action_slices_data = outputs
                # Debug: log what we got from eagenerate
                _debug_log(f"  [predict_action] Extracted from eagenerate: db_accept_lengths_data={db_accept_lengths_data}, type={type(db_accept_lengths_data)}")
            elif len(outputs) == 3:
                generated_ids, accept_lengths_data, draft_tokens_data = outputs
                # If use_db_action_slice is True but we only got 3 values, DB data is missing
                if use_db_action_slice:
                    _debug_log(f"  [predict_action] Warning: use_db_action_slice=True but eagenerate returned only 3 values, DB data missing")
                    db_accept_lengths_data = []
                    db_action_slices_data = []
            elif len(outputs) == 2:
                generated_ids = outputs[0]
            else:
                generated_ids = outputs[0] if len(outputs) > 0 else outputs
                # If use_db_action_slice is True but we got unexpected number of values
                if use_db_action_slice:
                    _debug_log(f"  [predict_action] Warning: use_db_action_slice=True but eagenerate returned {len(outputs)} values, DB data might be missing")
                    db_accept_lengths_data = []
                    db_action_slices_data = []
       ...
       # print(normalized_actions) 
        ...
        # 如果需要返回隐藏状态
        if return_hidden_states:
            if track_accept_length and isinstance(outputs, tuple) and len(outputs) == 3:
                # If tracking, outputs is (generated_ids, accept_lengths, draft_tokens)
                # We need to get hidden states separately
                tmp_hidden = None  # Hidden states not available in this case
            elif (len(outputs)==2):
                tmp_hidden = outputs[1]
            else:
                tmp_hidden=outputs.hidden_states if hasattr(outputs, 'hidden_states') else None
            # 使用前向传播获取隐藏状态
            first_layer_hidden = []
            last_layer_hidden = []
            if tmp_hidden is not None:
                #print(len(outputs.hidden_states))
                for i in range(len(tmp_hidden)):
                    last_layer_hidden.append(tmp_hidden[i][-1].cpu()[0])
                    first_layer_hidden.append(tmp_hidden[i][0].cpu()[0])
            # 返回三元组或二元组: (动作, 隐藏状态) 或 (动作, token_ids, 隐藏状态, 接受长度数据)
            #print(last_layer_hidden[0].shape)
            if track_accept_length:
                # IMPORTANT: If use_db_action_slice is True, always return DB tracking data
                if use_db_action_slice:
                    if db_accept_lengths_data is None:
                        db_accept_lengths_data = []
                        db_action_slices_data = []
                        _debug_log(f"  [predict_action] Warning: db_accept_lengths_data is None (return_hidden_states=True), returning empty lists")
                    return actions, predicted_action_token_ids, (first_layer_hidden, last_layer_hidden), accept_lengths_data, draft_tokens_data, db_accept_lengths_data, db_action_slices_data
                return actions, predicted_action_token_ids, (first_layer_hidden, last_layer_hidden), accept_lengths_data, draft_tokens_data
            return actions, predicted_action_token_ids,(first_layer_hidden,last_layer_hidden)
        
        # 如果追踪接受长度，返回额外数据
        if track_accept_length:
            # IMPORTANT: If use_db_action_slice is True, we should ALWAYS return DB tracking data
            # This ensures consistent return value structure (always 5 values when use_db_action_slice=True)
            if use_db_action_slice:
                # Ensure db_accept_lengths_data and db_action_slices_data are always lists (never None)
                if db_accept_lengths_data is None:
                    # DB retrieval failed or data not extracted - return empty lists
                    db_accept_lengths_data = []
                    db_action_slices_data = []
                    _debug_log(f"  [predict_action] Warning: db_accept_lengths_data is None, returning empty lists")
                else:
                    _debug_log(f"  [predict_action] Returning 5 values: db_accept_lengths_data={db_accept_lengths_data}")
                return actions, accept_lengths_data, draft_tokens_data, db_accept_lengths_data, db_action_slices_data
            return actions, accept_lengths_data, draft_tokens_data
    ...
    def eagenerate(
        ...
        **kwargs
        ...
        # Hook: Check if we should use DB action slice
        db_action_slice = kwargs.get('db_action_slice', None)
        use_db_action_slice = kwargs.get('use_db_action_slice', False)
        
        input_len = input_ids.shape[1]-1
        max_length = max_length - self.ea_layer.total_tokens - 10
        new_token = 0
        # Track accept lengths and draft tokens if requested
        accept_lengths = [] if track_accept_length else None
        draft_tokens_list = [] if track_accept_length else None
        # Track DB action slice accept length if using DB action slice
        db_accept_lengths = [] if (use_db_action_slice and track_accept_length) else None
        db_action_slices = [] if (use_db_action_slice and track_accept_length) else None
        
        # If using DB action slice, replace draft tokens BEFORE verification and use AR for subsequent tokens
        if use_db_action_slice and db_action_slice is not None:
            # Convert DB action slice ([-1,1] continuous values) to token IDs
            db_action_slice_np = np.array(db_action_slice) if not isinstance(db_action_slice, np.ndarray) else db_action_slice
            # Ensure it's the first action (shape should be (7,))
            if db_action_slice_np.ndim > 1:
                db_action_slice_np = db_action_slice_np[0]  # Take first action
            
            # Convert continuous action values [-1,1] to discrete token IDs
            # action_to_token_ids handles the conversion: [-1,1] -> bins -> token_ids
            db_action_slice_token_ids = self.action_to_token_ids(db_action_slice_np)
            # Convert to torch tensor
            db_action_slice_token_ids_torch = torch.from_numpy(db_action_slice_token_ids).long().to(input_ids.device)
            
            # Debug: log conversion info
            if track_accept_length:
                _debug_log(f"    [eagenerate] DB action slice ([-1,1]): {db_action_slice_np}")
                _debug_log(f"    [eagenerate] Converted to token IDs: {db_action_slice_token_ids}")
                _debug_log(f"    [eagenerate] Token IDs shape: {db_action_slice_token_ids_torch.shape}, max_new_tokens: {max_new_tokens}")
            
            # Store DB action slice for tracking (store the original [-1,1] values)
            if db_action_slices is not None:
                db_action_slices.append(db_action_slice_np.copy())
            
            # Replace draft_tokens with DB action slice BEFORE tree_decoding
            # Replace the first max_new_tokens (action_dim=7) positions in draft_tokens
            if len(db_action_slice_token_ids_torch) == max_new_tokens and draft_tokens.shape[1] >= len(db_action_slice_token_ids_torch):
                draft_tokens[0, :len(db_action_slice_token_ids_torch)] = db_action_slice_token_ids_torch
                if track_accept_length:
                    _debug_log(f"    [eagenerate] Replaced draft_tokens[:{len(db_action_slice_token_ids_torch)}] with DB action slice token IDs")
            else:
                if track_accept_length:
                    _debug_log(f"    [eagenerate] Warning: Token IDs length ({len(db_action_slice_token_ids_torch)}) != max_new_tokens ({max_new_tokens}) or draft_tokens shape incompatible")
            
            # Verify DB action slice: do tree_decoding and evaluate_posterior ONCE
            ...
            # Track accept length for DB action slice
            if track_accept_length:
                accept_len_value = accept_length.item() if isinstance(accept_length, torch.Tensor) else accept_length
                accept_lengths.append(accept_len_value)
                draft_tokens_list.append(candidates.cpu().numpy())
                if db_accept_lengths is not None:
                    db_accept_lengths.append(accept_len_value)
                    _debug_log(f"    [eagenerate] DB action slice verified: accept_length={accept_len_value}")
            
            # Update inputs after verification (this adds accepted tokens to input_ids)
            ...
            # Continue with normal tree decoding loop for remaining tokens
            # The tree decoding will naturally generate tokens autoregressively
            for idx in range(1, max_length):  # Start from 1 since we already did idx=0
                ...
                # Track accept length and draft tokens if requested
                if track_accept_length:
                    accept_lengths.append(accept_length.item() if isinstance(accept_length, torch.Tensor) else accept_length)
                    draft_tokens_list.append(candidates.cpu().numpy())
            ...
            # Normal tree decoding path (no DB action slice)
            for idx in range(max_length):
                ...
                if track_accept_length:
                    accept_lengths.append(accept_length.item() if isinstance(accept_length, torch.Tensor) else accept_length)
                    # Store the draft tokens (candidates) that were evaluated
                    draft_tokens_list.append(candidates.cpu().numpy())
        ...
        if not log:
            if track_accept_length:
                # IMPORTANT: If use_db_action_slice is True, ALWAYS return DB tracking data
                # even if db_accept_lengths is None (which means DB retrieval failed)
                if use_db_action_slice:
                    # Ensure db_accept_lengths and db_action_slices are always lists (never None)
                    if db_accept_lengths is None:
                        db_accept_lengths = []
                    if db_action_slices is None:
                        db_action_slices = []
                    _debug_log(f"  [eagenerate] Returning: db_accept_lengths={db_accept_lengths}, len={len(db_accept_lengths) if isinstance(db_accept_lengths, list) else 'N/A'}")
                    return input_ids[:,input_len+1:], accept_lengths, draft_tokens_list, db_accept_lengths, db_action_slices
                return input_ids[:,input_len+1:], accept_lengths, draft_tokens_list
            return input_ids[:,input_len+1:]
        else:
            if track_accept_length:
                if use_db_action_slice and db_accept_lengths is not None:
                    # Always return DB tracking data, even if empty list
                    return input_ids, new_token, idx, accept_lengths, draft_tokens_list, db_accept_lengths, db_action_slices
                return input_ids, new_token, idx, accept_lengths, draft_tokens_list
